package WifiWebUI::Controller::checkSSIDAvailability;
use utf8;
use open ':std', ':utf8';
binmode(STDOUT, ":utf8");
use Encode;
...
WifiWebUI::Controller::checkSSIDAvailability - Catalyst Controller
...
sub checkSSIDAvailability :Path('/checkSSIDAvailability') :Args(0) {
    my ($self, $c) = @_;
	...
	$c->stash->{_API_endpoint} = '/checkSSIDAvailability';
    ...
    my $configSSID = $configJSON->{wifi}->{ssid};
    my $testResult = "";

    for my $availableSSID (@filteredAvailableWifiNetworks) {
        if ($configSSID eq $availableSSID) {
	...
	$c->stash->{availability} = $testResult;
